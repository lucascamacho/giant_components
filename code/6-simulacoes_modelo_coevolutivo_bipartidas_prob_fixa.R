...
networks_files <- list.files(here("data/bip_matrices"), pattern="*.txt") #Network names
networks_path <- list.files(here("data/bip_matrices"), pattern="*.txt", full.names=TRUE) #Networks file paths
...
sfInit(parallel = TRUE, cpus = 20, type = "SOCK") #Initializing parallel clusters
